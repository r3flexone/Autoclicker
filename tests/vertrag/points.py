...
stehen (und `save()` schrieb sie mit), `edit_sequence()` in der Konsole
...
try:
    ...
    # `load()` ersetzt die Punkte, `new()` liess sie stehen - und
    # `save()` schreibt `self.points` in die Datei. Eine frisch angelegte
    ...
    # stehen - und weil `_detection_rename` die alte Datei absichtlich
    ...
    # **Ein Umbenennen, das klont, ist keins.** `_scan_load()` sieht den Ordner
