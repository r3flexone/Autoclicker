...
# `_punkte_schreibfertig()` ist mit der Migrationskette entfallen. Es brachte Punkte,
# die `_seq_v3_to_v4` gerade angelegt hatte, in dieselbe Form wie die aus der Datei -
# gebraucht wurde es nur an der einen Stelle, die points.json nachschrieb. Die
# regulaere Reinigung von points.json macht weiterhin der Round-Trip (`_rt_points`).
...
def sweep(write: bool = False, punkte: Optional[list] = None) -> SweepErgebnis:
    ...
    # Die Punkte gehen weiterhin als Kontext durch: die Round-Trip-Funktionen brauchen
    # sie, um die Koordinaten einer Sequenz aufzuloesen (in der Datei stehen nur IDs).
    # Was hier entfallen ist, ist das Nachschreiben: solange die Sequenz-Kette leer ist,
    # legt keine Migration mehr Punkte an.
