...
Höchstens alle 200 ms ein Schreibvorgang (`immediately=True` umgeht die Drossel);
Schreibfehler werden geschluckt — ein Beobachter darf den Lauf nie stören.

**Geschrieben wird aus mehreren Threads** — Worker, Lebenszeichen
(`start_heartbeat`) und der asynchrone Boss-Thread (dessen Pause meldet sich
hier). Deshalb sitzt alles hinter `_lock`: ein `update()` waehrend `compact_json`
dasselbe Dict durchlaeuft, wirft `RuntimeError`, und der fiele durch den
`except` unten.
"""
...
_last = 0.0
_state: dict = {}
# Reentrant, weil `waiting_for()` ueber `write_status()` schreibt. Nie unter
# `state.lock` nehmen — `_counters()` sperrt ihn hier drin (Reihenfolge:
# erst `_lock`, dann `state.lock`).
_lock = threading.RLock()

# Wie oft das Lebenszeichen schreibt. Der Leser erklaert einen Lauf nach 5 s
# ohne Stempel fuer verwaist — eine Sekunde laesst Luft fuer eine volle Platte.
...
def write_status(state, part: dict, immediately: bool = False) -> None:
    ...
    `immediately=True` umgeht die Drossel — für Ereignisse, die man nicht
    verpassen darf (Start, Phasen- und Zykluswechsel). Eingeführt wird immer,
    gedrosselt nur das Schreiben: sonst ginge die Information eines verworfenen
    Aufrufs verloren.
    """
    global _last
    with _lock:
...
def heartbeat(state) -> None:
    ...
    Lebenszeichen sähe genau der Lauf tot aus, der gerade wartet. Kostet
    nichts — die Drossel lässt höchstens fünf Schreibvorgänge pro Sekunde durch.
    """
    write_status(state, {})


def start_heartbeat(state, interval: float = HEARTBEAT_INTERVAL):
    """Hält den Stempel frisch, solange der Lauf lebt — unabhängig vom Worker.

    Die Warteschleifen rufen `heartbeat()` selbst, aber ein einzelner Aufruf
    kann den Worker länger aufhalten als jede Schleife: ein LLM-Boss-Scan an
    einem kalten Modell blockiert über eine Minute in EINER HTTP-Antwort, ein
    erster OCR-Aufruf lädt Modelle nach. Der Leser erklärte den Lauf nach 5 s
    für verwaist, und im Studio stand „kein Hauptprozess" über einem Lauf,
    der nur wartete. Verwaist heisst „der Prozess ist weg" — und mit dem
    Prozess stirbt auch dieser Thread.

    Gibt die Funktion zurück, die ihn anhält; der Worker ruft sie VOR
    `finish_run()` — sonst schriebe ein letzter Takt über die Zusammenfassung.
    Sie wartet, bis der Thread wirklich steht.
    """
    ...
    thread = threading.Thread(target=beat, daemon=True, name="run-heartbeat")
    thread.start()
