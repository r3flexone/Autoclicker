...
from .model import load_slots, save_slots, next_slot_name, normalize_region
...
class ScanStudioApp:
    ...
    def __init__(self, image, virtual_left: int, virtual_top: int,
                 slots_file: str, max_canvas=(1100, 760)):
        self.image = image
    ...
    def refresh_slot_list(self) -> None:
        ...
        for name in self.slots:
            label = ("» " if name == self.selected else "") + name
    ...
    def _on_save(self, *_):
        if save_slots(self.slots, self.slots_file):
            self._set_status(f"{len(self.slots)} Slot(s) gespeichert.")
        else:
